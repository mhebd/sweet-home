(() => {
  ...
  function setInfo(e) {
    ...
    data.background = domEl.editForm.background.value;
  ...
  function getBackground() {
    ...
    return backgroundIT = JSON.parse(data).background !== null ? JSON.parse(data).background : 'sea beatch';
  }
  ...
  async function setBackground() {
    const query = backgroundIT;
    const res = await fetch(`https://api.unsplash.com/photos/random?client_id=OrhxU0W9Z03nJCKjuq4J1YTv8xXjut-zRU7m5KdTrhU&orientation=landscape&query=${query}`);
    const data = await res.json();
  ...
  function others() {
    ...
    domEl.editForm.background.value= backgroundIT;
  };
